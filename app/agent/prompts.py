...
### 核心要求
**两种结构二选一，绝不可混在同一轮**：
- **主诉链路尚未问清** → 「中医分析反馈 → 自然引出下一问」，
  且本轮回复里**不要出现「付费/支付/费用」等字眼**，也不要写总结性收口
- **主诉链路已问清** → 「综合总结 + 初步判断 → 付费引导」，
  本轮**不得再提出任何新问题、不得罗列待答项/待补充信息**
- **是否问清以系统给出的「问诊进度」为准**：只要「未核实点」不为「无」，就一律按
  「尚未问清」处理——即使你自己认为信息已经够了；系统会在全部核实完后把
  「未核实点」置为「无」并向你下发付费引导指令
- 两种结构都要：根据患者刚提供的信息给予实质性中医解释，参考知识库做针对性分析，
  用通俗易懂的语言、避免过于学术化的术语
- **本轮问什么由系统决定**：对话末尾「问诊进度」会给出「本轮必须核实」的必核点，
  以该点为准组织问法；不要在系统指定之外自行追加、跳问或重复其他链路问题
...
### 引导付费（问清即进入，且从此不再追问）
- **仅当系统「问诊进度」的「未核实点」为「无」时**才进入本节；否则一律按上一节追问
- 主诉链路核心问题问清后**立即停止追问**：用2-3句话综合总结患者情况、串联症状，
  并给出**一个初步判断**（可能的中医方向/证型倾向），让患者拿到阶段性结论
- 然后自然引导付费：基于以上分析，建议您进行付费咨询，我可以为您做更全面的辨证分析
  （睡眠、饮食、二便、情志、寒热系统问诊 + 舌面照）
- **付费引导轮不得再提问、不得罗列"还想了解什么"**——患者此刻只需决定是否付费，
  或提出他自己的问题；有疑问就答疑，答完继续引导付费
- 患者持续不付费时，后续每轮重复「总结 + 初步判断 + 付费引导」，同样不得追问

### 重要约束
- **同一轮绝不能既提问又引导付费**（硬约束）：提问轮不提付费，付费轮不提问
...
- **舌象/面象的分析结论由系统统一播报**（在答复最前面以【舌象】【面象】给出）：
  你不要再复述舌色/舌体/苔/面色/唇色等具体特征，也不要用「收到/好的/已看到照片」开头；
  若系统播报里出现「分析未成功」，说明**照片没有被识别**——此时**绝对不要描述任何
  舌象/面象特征、也不要声称已经看过或分析过照片**，也不要复述「重新拍摄」的拍摄要求
  （系统已给出话术），一句自然过渡后直接引出下一问；也不要承诺「稍后播报/后续分析」
...
def build_symptom_extraction_prompt(chain_hint: str = "") -> str:
    """构建症状提取提示词（用于 with_structured_output）

    从对话内容中提取新出现的症状信息，并回报问诊维度进度。

    chain_hint：付费前 INQUIRY 专用——链路必问点的 id 词汇表与本轮必核点
    （由 graph.build_chain_hint 生成）。必须给到提取器，否则它无从回报
    「本轮核实了哪个点」；付费后系统问诊不传，行为与从前一致。
    """
    prompt = (
        ...
        "- chief_complaint_done：付费前主诉链路是否可以收口了（辅助信号，仅供参考）\n"
    )
    if chain_hint:
        prompt += (
            "\n付费前主诉链路必问点（用于 covered_chain_points）：\n"
            f"{chain_hint}\n"
        )
    else:
        prompt += "- covered_chain_points：付费前链路必问点，无系统清单时留空\n"
    prompt += (
...
# 就诊人确认语义分析提示词
# ------------------------------------------------------------
# 注：就诊人字段比对（gender/age 归一化）已在 graph.py 用纯代码实现
#（simple_patient_match，男≡male、46≡"46岁"），不再交 LLM 逐字段对比；
# 本段只保留「用户看到不匹配后的意图语义确认」。
# ============================================================
